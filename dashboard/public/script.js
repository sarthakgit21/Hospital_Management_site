  document.addEventListener("DOMContentLoaded", function () {
    ...
    const createPopup = (options) => {
      ...
      // Cool background gradient and glassmorphism effect
      popup.style.background =
        "linear-gradient(135deg, rgba(255, 255, 255, 0.1), rgba(255, 255, 255, 0.05))";
      popup.style.backdropFilter = "blur(10px)";
      popup.style.color = options.mainTextColor;
      popup.style.borderRadius = "12px"; // Reduced border radius
      // Increased shadow
      ...
      mainText.style.textShadow = "none"; // No shadow
      ...
      subText.style.textShadow = "none"; // No shadow
      ...
      // Append the popup to the root element
    ...
    const fetchAndShowPopups = async () => {
      try {
        ...
        templates.forEach((data, index) => {
          ...
          const triggerScrollPercentage = triggerEvent;
